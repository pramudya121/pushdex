...
export const WaveBackground: React.FC<{ className?: string }> = memo(({ className = '' }) => {
  return (
    <div className={`fixed inset-0 overflow-hidden pointer-events-none ${className}`}>
      ...
      {/* Glow orbs with purple accent */}
      <div 
        className="absolute top-1/4 left-1/4 w-[500px] h-[500px] rounded-full blur-[120px] opacity-25 animate-pulse"
      ...
      />
      <div 
        className="absolute top-1/2 right-1/3 w-[300px] h-[300px] rounded-full blur-[80px] opacity-15"
        style={{ background: 'radial-gradient(circle, hsl(330, 100%, 65%) 0%, transparent 70%)' }}
      />
    </div>
  );
});
